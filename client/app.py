...
# Helper to safely parse responses that might not be JSON
def _parse_response_safely(response):
    try:
        return response.json()
    except ValueError:
        text = response.text.strip() if hasattr(response, "text") else ""
        if not text:
            return {"detail": f"Empty response body (HTTP {response.status_code})."}
        # Try to include something useful without overwhelming the UI
...
def signup_user(username, password, role):
    try:
        ...
        return response.status_code, _parse_response_safely(response)
